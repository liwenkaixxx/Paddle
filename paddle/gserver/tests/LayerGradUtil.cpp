  ...
  for (size_t i = 0; i < testConf.inputDefs.size(); ++i) {
    ...
    switch (testConf.inputDefs[i].inputType) {
      ...
      case INPUT_SELF_DEFINE_DATA: {
        size_t height = testConf.inputDefs[i].selfDefineData->getHeight();
        size_t width = testConf.inputDefs[i].selfDefineData->getWidth();
        ...
        data.value->copyFrom(*testConf.inputDefs[i].selfDefineData);
        data.grad->zeroMem();
